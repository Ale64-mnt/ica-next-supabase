#!/usr/bin/env python3
"""
Script Automatico per Inserire Alert Multilingua in Supabase
...
# Dati per ogni alert in diverse lingue
ALERTS_DATA = [
    # Italiano
    {
...
* Si vous avez déjà payé ou transmis des données : **contactez votre banque** et **déposez plainte**.
...
def main():
    ...
    print("=" * 60)
    print("🤖 SCRIPT AUTOMATICO INSERIMENTO ALERT ")
    ...
    print(f"📝 Trovati {len(ALERTS_DATA)} alert da inserire")
    print("-" * 60)
    ...
    for i, alert in enumerate(ALERTS_DATA, 1):
        ...
        print(f"🔄 Inserimento {i}/{len(ALERTS_DATA)}: {alert['title']} ({locale})...")
        
        # Inserimento
        success, result = insert_alert(alert_data)
        
        if success:
            print(f"   ✅ SUCCESSO: {alert['title']}")
            success_count += 1
        else:
            print(f"   ❌ ERRORE: {result}")
            errors.append(f"{alert['title']} ({locale}): {result}")
    
    # Riepilogo
    print("\n" + "=" * 60)
    print("📊 RIEPILOGO INSERIMENTO")
    print("=" * 60)
    print(f"✅ Alert inseriti con successo: {success_count}/{len(ALERTS_DATA)}")
    ...
    if errors:
        ...
        print("🎉 Tutti gli alert sono stati inseriti correttamente!")
